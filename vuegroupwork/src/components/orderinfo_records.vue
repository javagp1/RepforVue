						...
						<tbody>
							<tr v-for="(order,index) in orderinfoes" >
								...
								<td>
									{{order.ofid}} 
									&nbsp;&nbsp;&nbsp;
									<button class="btn btn-success" @click="toAliPay(order.ofid)">付款</button>
								</td>
								<td>{{order.ofdate}}</td>
								<td>
	...
	export default{
		...
		methods:{
			toAliPay(orderid){
				...
				window.open("http://192.168.1.14:8088/springmvc190806/ali/pay?orderid="+orderid);
			...
			getOrderInfoes(){
				...
				var ob=this;
				var url="http://192.168.1.14:8088/springmvc190806/ajaxctr/getorderinfoes";
				$.ajax(url,{
				dataType:"json",
